#!/bin/bash
# run_process.sh - Process pending entries with logging
#
# Usage:
#   ./scripts/run_process.sh                    # Process all pending
#   ./scripts/run_process.sh --entry-id <id>    # Process specific entry
#   ./scripts/run_process.sh --date 2024-01-15  # Process by date
#   ./scripts/run_process.sh --sequential       # Process sequentially
...
RUN_NAME="process_${TIMESTAMP}"
RUN_PATH="$RUN_DIR/$RUN_NAME"

mkdir -p "$RUN_PATH"

LOG_FILE="$RUN_PATH/process.log"
ERROR_LOG="$RUN_PATH/errors.log"

echo "=============================================="
echo "AgroAmigo Processing Run"
echo "=============================================="
