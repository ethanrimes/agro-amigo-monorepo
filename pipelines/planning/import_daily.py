...
def run(cur,iso):
 ...
 cur.executemany('INSERT INTO daily_price(observed_on,product_name,market_name,product_id,price,change_percent,document_id,source_page,source_locator) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s) ON CONFLICT(observed_on,product_name,market_name) DO UPDATE SET price=excluded.price,document_id=excluded.document_id,source_page=excluded.source_page',[(d,n,m,pid,price,v,extract,k//24+1,f'Boletín diario, fila {row}, columna {col}') for k,(d,n,m,pid,price,v,row,col) in enumerate(values)])
