...
with psycopg.connect(host=config['host'],dbname=config['database'],user=config['user'],password=config['password'],sslmode='verify-full',sslrootcert=certifi.where(),connect_timeout=15) as db:
 for table in ['price_observation','coffee_reference','coffee_factor','exchange_rate','input_price','daily_price']:
  n,missing,outside=db.execute(f'SELECT count(*),count(*) FILTER(WHERE document_id IS NULL),count(*) FILTER(WHERE observed_on>%s) FROM {table}',(TODAY,)).fetchone();assert missing==outside==0,(table,missing,outside);print(table,n,'dated and traceable')
 total=0;size=0
 ...
 assert db.execute('SELECT count(*) FROM seasonal_year WHERE reference_year>=EXTRACT(YEAR FROM CURRENT_DATE) OR jsonb_array_length(monthly_prices)<>12').fetchone()[0]==0
 ...
 assert db.execute("SELECT price FROM daily_price WHERE observed_on=DATE '2026-09-04' AND product_name=%s AND market_name=%s",('Habichuela','Montería, Mercado del Sur')).fetchone()[0]==6250
