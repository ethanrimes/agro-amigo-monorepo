...
`lib/main.dart` provides a WKWebView client with persistent local farm/offer/scenario data, native back and swipe navigation, connection retry, source downloads through the iOS share sheet / Save to Files, and JSON scenario export. External HTTPS sources open in the system browser. Only the exact Azure HTTPS origin stays inside the app. There are no database credentials, arbitrary JavaScript bridges, geolocation permissions, or background push notifications in the native client. Prices and documents require internet access.
...
The integration test uses the public Azure demo and a dedicated test simulator. It creates a local Pitalito example profile, checks real PDF rendering and native back navigation, and captures the native scenario share sheet for inspection in `../../artifacts`. It does not upload private offers or publish data.
