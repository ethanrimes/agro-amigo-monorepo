...
for (const viewport of [
  ...
  test(`phone layout fills ${viewport.width}x${viewport.height} with equal tabs`, async ({
    page,
    isMobile,
  }, info) => {
    ...
    const layout = await page.evaluate(() => {
      ...
      return {
        ...
        nav: box(".mobile-nav"),
        shell: box(".main-shell"),
        content: box(".app-content"),
        tabs: [...document.querySelectorAll(".mobile-nav a")].map((a) =>
    ...
    await page.screenshot({
      path: resolve(
        __dirname,
        "../../../artifacts",
        `layout-${info.project.name}-${viewport.width}.png`,
      ),
    });
    await nav.getByRole("link", { name: "Mi finca" }).click();
    await expect(page).toHaveURL(/\/farm$/);
    await expect(nav.getByRole("link", { name: "Mi finca" })).toHaveAttribute(
      "aria-current",
      "page",
    );
...
test("larger phone text keeps navigation and form controls usable", async ({
  page,
  isMobile,
}) => {
