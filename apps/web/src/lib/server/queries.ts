...
export async function productDetail(id: string, region = "") {
  ...
  const [markets, history] = await Promise.all([
    db.query<MarketPrice>(
      `SELECT DISTINCT ON (m.id) m.*, o.price, o.min_price, o.max_price, o.observed_on AS date, o.source_url, o.document_id, o.source_locator, o.period, o.unit FROM price_observation o JOIN market m ON m.id=o.market_id WHERE product_id=$1 AND ${WINDOW} AND ($2='' OR m.region=$2) ORDER BY m.id, o.observed_on DESC`,
      [id, region],
    ),
    db.query<Point>(
...
export async function coffee(): Promise<Coffee | null> {
  ...
  const [reference, markets, factors, exchange] = await Promise.all([
    ...
    db.query<MarketPrice>(
      `SELECT DISTINCT ON (m.id) m.*, o.price, o.min_price, o.max_price, o.observed_on AS date, o.source_url, o.document_id, o.source_locator, o.period, o.unit FROM price_observation o JOIN market m ON m.id=o.market_id WHERE source_id='fnc' AND ${WINDOW} ORDER BY m.id, o.observed_on DESC`,
    ),
